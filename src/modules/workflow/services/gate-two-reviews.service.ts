...
type GateTwoReviewAction = 'QA' | 'FINANCE' | 'GM';

@Injectable()
export class GateTwoReviewsService {
  constructor(
  ...
  async recordReview(
    productId: string,
    action: GateTwoReviewAction,
    actor: AuthenticatedUser,
    input: { comment?: string; overrideReason?: string },
  ): Promise<{ auditLog: AuditLogRecord; review: GateTwoReviewRecord }> {
    ...
    if (product.currentStage !== ProductStage.STAGE_2) {
      throw new BadRequestException({
        ...
        message: `Gate 2 review checkpoints can only be updated while product ${productId} is in Stage 2.`,
      });
    }
    ...
    const existingReview = await this.gateTwoReviewsRepository.findByProductId(productId);
    const client = await this.databaseService.getClient();

    try {
      await client.query('BEGIN');

      const review = await this.gateTwoReviewsRepository.upsert(
    ...
    switch (action) {
      case 'QA':
        return {
          productId,
          qaComment: comment,
          qaReviewCompletedAt: timestamp,
          qaReviewedByUserId: actorUserId,
        };
      case 'FINANCE':
        return {
  ...
  private mapAuditAction(action: GateTwoReviewAction): AuditAction {
    switch (action) {
      case 'QA':
        return AuditAction.QA_REVIEW_COMPLETED;
      case 'FINANCE':
